...
export const getStoreProductPage = async (params: PageParam) => {
  return await request.post({url: `/product/store-product/cashier/all/products`, data: params })
}
...
export const getStoreProduct = async (id: number) => {
  return await request.get({url: `/product/store-product/cashier/detail?productId=` + id })
}
